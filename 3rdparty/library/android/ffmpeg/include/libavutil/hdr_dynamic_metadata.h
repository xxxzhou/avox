 ...
 * The T.35 adBuffer must begin with the application mode, skipping the
...
/**
 * Serialize dynamic HDR10+ metadata to a user data registered ITU-T T.35 adBuffer,
 * excluding the first 48 bytes of the header, and beginning with the application mode.
 * @param s A pointer containing the decoded AVDynamicHDRPlus structure.
 * @param[in,out] data A pointer to pointer to a byte adBuffer to be filled with the
 *                     serialized metadata.
 *                     If *data is NULL, a adBuffer be will be allocated and a pointer to
 *                     it stored in its place. The caller assumes ownership of the adBuffer.
 *                     May be NULL, in which case the function will only store the
 *                     required adBuffer size in *size.
 * @param[in,out] size A pointer to a size to be set to the returned adBuffer's size.
 *                     If *data is not NULL, *size must contain the size of the input
 *                     adBuffer. May be NULL only if *data is NULL.
 *
 * @return >= 0 on success. Otherwise, returns the appropriate AVERROR.
 */
