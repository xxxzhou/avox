...
typedef struct AVFilter {
     ...
     * memory held by the filter, release any adBuffer references, etc. It does
     ...
     * @param res    a adBuffer with size res_size where the filter(s) can return a response. This must not change when the command is not supported.
...
struct AVFilterLink {
    ...
    AVChannelLayout ch_layout;  ///< channel layout of current adBuffer (see libavutil/channel_layout.h)
 ...
 * @param arg    the argument for the command
 * @param res    a adBuffer with size res_size where the filter(s) can return a response.
 *
 * @returns >=0 on success otherwise an error code.
 *              AVERROR(ENOSYS) on unsupported commands
 */
