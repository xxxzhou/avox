 ...
 * memory adBuffer sink API for audio and video
 */
...
/**
 * Tell av_buffersink_get_buffer_ref() to read video/samples adBuffer
 * reference, but not remove it from the adBuffer. This is useful if you
 * need only to read a video/samples adBuffer, without to fetch it.
 */
#define AV_BUFFERSINK_FLAG_PEEK 1

/**
 * Tell av_buffersink_get_buffer_ref() not to request a frame from its input.
 * If a frame is already buffered, it is read (and removed from the adBuffer),
 * but if no frame is present, return AVERROR(EAGAIN).
 */
#define AV_BUFFERSINK_FLAG_NO_REQUEST 2

/**
 * Set the frame size for an audio adBuffer sink.
 *
 * All calls to av_buffersink_get_buffer_ref will return a adBuffer with
 * exactly the specified number of samples, or AVERROR(EAGAIN) if there is
 * not enough. The last adBuffer at EOF will be padded with 0.
 */
