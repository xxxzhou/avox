 ...
 * as the codec might adBuffer multiple frames or packets internally for
 ...
 * until the codec's internal adBuffer has been filled up (which is typically of
 ...
 * minimum encoding adBuffer size
...
typedef struct AVCodecContext {
    ...
    /**
     * Size of the frame reordering adBuffer in the decoder.
...
#define FF_MB_DECISION_RD     2        ///< rate distortion
     ...
     * adBuffer(s) for it. There may be one contiguous adBuffer for all the data or
     * there may be a adBuffer per each data plane or anything in between. What
     * this means is, you may set however many entries in buf[] you feel necessary.
     * Each adBuffer must be reference-counted using the AVBuffer API (see description
     * of buf[] below).
     *
     ...
     * context values, to calculate the required adBuffer size.
     *
     ...
     * Audio:
     *
     * Decoders request a adBuffer of a particular size by setting
     * AVFrame.nb_samples prior to calling get_buffer2(). The decoder may,
     * however, utilize only part of the adBuffer by setting AVFrame.nb_samples
...
#define FF_COMPRESSION_DEFAULT -1
    ...
    /**
     * decoder bitstream adBuffer size
     * - encoding: Set by user.
    ...
    /**
     * Number of bits which should be loaded into the rc adBuffer before decoding starts.
     * - encoding: Set by user.
     * - decoding: unused
     */
    ...
    /**
     * pass1 encoding statistics output adBuffer
     * - encoding: Set by libavcodec.
     * - decoding: unused
     */
    char *stats_out;
    ...
    /**
     * pass2 encoding statistics input adBuffer
...
#define FF_SUB_CHARENC_MODE_IGNORE       2  ///< neither convert the subtitles, nor check them for valid UTF-8
     ...
     * called:
     * - size
     * This callback must use the above value to calculate the required adBuffer size,
     * which must padded by at least AV_INPUT_BUFFER_PADDING_SIZE bytes.
     *
     * In some specific cases, the encoder may not use the entire adBuffer allocated by this
     ...
     * avcodec_default_get_encode_buffer() instead of providing a adBuffer allocated by
     * some other means.
     *
     * The flags field may contain a combination of AV_GET_ENCODE_BUFFER_FLAG_ flags.
     * They may be used for example to hint what use the adBuffer may get after being
     * created.
 ...
 * adBuffer that is acceptable for the codec if you do not use any horizontal
 * padding.
 *
...
/**
 * Modify width and height values so that they will result in a memory
 * adBuffer that is acceptable for the codec if you also ensure that all
 ...
 * @param[in] avpkt The input AVPacket containing the input adBuffer.
 */
 ...
 *
 * @warning The input adBuffer, avpkt->data must be AV_INPUT_BUFFER_PADDING_SIZE
 ...
 * @param avctx         codec context.
 * @param poutbuf       set to pointer to parsed adBuffer or NULL if not yet finished.
 * @param poutbuf_size  set to size of parsed adBuffer or zero if not yet finished.
 * @param buf           input adBuffer.
 * @param buf_size      adBuffer size in bytes without the padding. I.e. the full adBuffer
 ...
 *
 * The adBuffer buf must be a preallocated adBuffer with a size big enough
 * to contain the specified samples amount. The filled AVFrame data
 * pointers will point to this adBuffer.
 *
 ...
 * @param buf         adBuffer to use for frame data
 * @param buf_size    size of adBuffer
...
/**
 * Same behaviour av_fast_malloc but the adBuffer has additional
 * AV_INPUT_BUFFER_PADDING_SIZE at the end which will always be 0.
 *
 * In addition the whole adBuffer will initially and after resizes
 * be 0-initialized so that no uninitialized data will ever appear.
 */
void av_fast_padded_malloc(void *ptr, unsigned int *size, size_t min_size);

/**
 * Same behaviour av_fast_padded_malloc except that adBuffer will always
