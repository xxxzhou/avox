...
#include "avox/layer/VLayer.hpp"

namespace avox {

typedef std::shared_ptr<VkWrapBuffer> VulkanBufferPtr;
typedef std::shared_ptr<VkTexture> VulkanTexturePtr;

// 如果CPU/GPU参数对应,并且不会导致graph重启
#define AVOX_VULKAN_PARAMETUPDATE()                                             \
protected:                                                                     \
...
class VkLayer : public VLayer, public VkContextRef {
  friend class VkPipeGraph;

public:
  VkLayer(/* args */);
  virtual ~VkLayer() override;
...
protected:
  ...
  // 默认提供一个参数buf
  std::unique_ptr<VkWrapBuffer> constBuf = nullptr;
  std::unique_ptr<UBOLayout> layout = nullptr;
  std::unique_ptr<VkShader> shader = nullptr;
  ...
  std::string glslPath = "";
  // 是否需要调用clearColor,这个需要TRANSFER_DST 
  bool bMustClear = false;

protected:
  NodePtr<VkLayer> getNode();
  // 当前pipegraph正在使用的commandbuffer
...
protected:
  ...
  void createOutTexs();
  void clearColor(vec4f color = {0.0, 0.0, 0.0, 1.0});
...
class VkGroupLayer : public VkLayer, public Group<VkLayer> {
public:
  VkGroupLayer(/* args */) = default;
  virtual ~VkGroupLayer() = default;

protected:
  virtual void onInitGraph() final;
  // GroupLayer不需要创建pipeline
  virtual void onInitPipe() override;
  virtual void onCommand() override;

protected:
  // 初始化时请指定
  virtual void onInitGroup() {};
  //
  virtual void onVisible(bool visible) override;
};

using VkPipeNode = PipeNode<VkLayer>;

template <typename T> using VKTNodePtr = TNodePtr<T, VkLayer>;
