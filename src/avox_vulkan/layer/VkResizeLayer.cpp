...
VkResizeLayer::VkResizeLayer(ImageType imageType) {
  ...
  if (imageType == ImageType::r8) {
    ...
    glslPath = "glsl/resizeUI1.comp.spv";
  }
  paramet.bLinear = 1;
...
bool VkResizeLayer::getSampled(int inIndex) {
  if (inIndex == 0 && imageType != ImageType::r16) {
    return true;
  }
  return false;
}
...
VkSizeScaleLayer::VkSizeScaleLayer(ImageType imageType_) {
...
void VkSizeScaleLayer::onInitLayer() {
  ...
  outFormats[0].height = inFormats[0].height * paramet.fy;
...
CVkGammaLayer::CVkGammaLayer(/* args */) {
