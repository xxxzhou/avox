...
VkSeparableLayer::VkSeparableLayer(ImageType imageType_) {
...
void VkSeparableLayer::updateBuffer(std::vector<float> data) {
  ...
  kernelBuffer = std::make_unique<VkWrapBuffer>();
  // kernelBuffer 是独立的 VkWrapBuffer, 不继承 layer 的 vkDevice,
  // 必须显式 setVkContext, 否则 vkDevice=VK_NULL_HANDLE, vkCreateBuffer 崩
  kernelBuffer->setVkContext(vkPipeGraph);
...
void VkSeparableLayer::onInitGroup() {
...
void VkSeparableLayer::onInitPipe() {
  ...
  auto computePipelineInfo = createComputePipelineInfo(
...
void VkSeparableLinearLayer::onInitNode() {
  rowLayer = vkPipeGraph->addNode<VkSeparableLayer>(imageType);
  rowLayer->addLine(getNode(), 0, 0);
  setStartNode(rowLayer);
}
...
void VkBoxBlurSLayer::onInitLayer() {
  ...
  rowLayer->get()->updateBuffer(karrayX);
...
void VkGaussianBlurSLayer::onInitLayer() {
  ...
  rowLayer->get()->updateBuffer(karray);
  updateBuffer(karray);
}
