...
UBOLayout::~UBOLayout() {
  if (vkDevice) {
    ...
    for (auto i = 0; i < size; i++) {
      vkDestroyDescriptorSetLayout(vkDevice, descSetLayouts[i], nullptr);
...
void UBOLayout::generateLayout(int32_t constSize) {
  ...
  AVOX_VULKAN_LOG(
      vkCreateDescriptorPool(vkDevice, &descriptorPoolInfo, nullptr, &descPool),
      "create descriptor pool fail");
  descSetLayouts.resize(size);
  descSets.resize(size);
  // layout (set=M, binding=N) uniform sampler2D textures[c];
  // M=x,N=i,binding=descriptorCount
  for (auto x = 0; x < size; x++) {
    ...
    for (auto i = 0; i < group.size(); i++) {
      ...
      // 大于1表示一个数组 uniform sampler2D textures[];
    ...
    AVOX_VULKAN_LOG(vkCreateDescriptorSetLayout(vkDevice, &descriptorLayoutInfo,
                                               nullptr, &descSetLayouts[x]),
                   "create descriptor set layout fail");
    // 生成VkDescriptorSet
    std::vector<VkDescriptorSetLayout> layouts(groupSize[x], descSetLayouts[x]);
    ...
    AVOX_VULKAN_LOG(
        vkAllocateDescriptorSets(vkDevice, &descAllocInfo, descSets[x].data()),
        "allocate descriptor sets fail");
  }
  // 生成pipelineLayout
  ...
  AVOX_VULKAN_LOG(vkCreatePipelineLayout(vkDevice, &pipelineLayoutCreateInfo,
                                        nullptr, &pipelineLayout),
                 "create pipeline layout fail");
}
...
void UBOLayout::updateSetLayout(uint32_t groupIndex, uint32_t setIndex, ...) {
  ...
  for (auto i = 0; i < group.size(); i++) {
    ...
    default:
      LOGFLF(LogLevel::warn, "unknow descriptor type");
      break;
    }
  ...
  va_end(args);
  vkUpdateDescriptorSets(vkDevice, static_cast<uint32_t>(writes.size()),
...
void UBOLayout::updateSetLayout(uint32_t groupIndex, uint32_t setIndex,
  ...
  for (auto i = 0; i < group.size(); i++) {
    ...
    // pImageInfo里的所有成员
    case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
      write.pImageInfo = static_cast<VkDescriptorImageInfo *>(bufferInfos[i]);
      break;
    case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
    case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
    case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC:
    case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
      write.pBufferInfo = static_cast<VkDescriptorBufferInfo *>(bufferInfos[i]);
      break;
    case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
      write.pTexelBufferView = static_cast<VkBufferView *>(bufferInfos[i]); 
    default:
      LOGFLF(LogLevel::warn, "unknow descriptor type");
      break;
    }
...
void createDefaultFixPipelineState(FixPipelineState &fix) {
  // ---图元装配
  ...
  fix.dynamicState.sType = VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO;
  fix.dynamicState.flags = 0;
  fix.dynamicState.pDynamicStates = fix.dynamicStateEnables.data();
  fix.dynamicState.dynamicStateCount = (uint32_t)fix.dynamicStateEnables.size();
}

VkPipelineShaderStageCreateInfo createShader(
#if __ANDROID__
    AAssetManager *assetManager,
#endif
    VkDevice vkDevice, const std::string &fileName,
    VkShaderStageFlagBits stage) {
  VkPipelineShaderStageCreateInfo shaderStage = {};
  shaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
  shaderStage.stage = stage;
#if defined(__ANDROID__)
  shaderStage.module = loadShader(assetManager, fileName.c_str(), vkDevice);
#else
  shaderStage.module = loadShader(fileName.c_str(), vkDevice);
#endif
  shaderStage.pName = "main";
  return shaderStage;
}
VkComputePipelineCreateInfo
createComputePipelineInfo(VkPipelineLayout layout,
