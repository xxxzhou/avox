...
// atomic: 统一视图内聚后渲染线程每帧读胜者, 激活操作在播放器线程写。
class SubtitleSlots {
 public:
  enum class Slot { none, track, file, asr };

  Slot active() const { return active_.load(std::memory_order_relaxed); }
  ...
  Slot activate(Slot slot) {
    ...
    const Slot prev = active_.exchange(slot, std::memory_order_relaxed);
    return prev;
  }
  ...
  Slot deactivateIf(Slot slot) {
    const Slot prev = active_.load(std::memory_order_relaxed);
    if (prev == slot) {
      active_.store(Slot::none, std::memory_order_relaxed);
    }
    return prev;
  }
  ...
  Slot reset() {
    return active_.exchange(Slot::none, std::memory_order_relaxed);
  }

 private:
  std::atomic<Slot> active_{Slot::none};
};
