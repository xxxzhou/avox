...
import avox.android.library.swig.ImageFormat;
import avox.android.library.swig.ImageType;

// 把Vk的纹理结果渲染到opengl上
public class GLVkVideoRender implements GLSurfaceView.Renderer {
    private IRenderContext eglContext = null;
    ...
    public void init(GLSurfaceView glSurfaceView, IGLRenderObserver glCopyTexture) {
    ...
    public void onSurfaceCreated(GL10 gl10, EGLConfig eglConfig) {
        EGLContext mEGLContext = EGL14.eglGetCurrentContext();
        long natContext = mEGLContext.getNativeHandle();
        eglContext = AvoxWrapper.createGLESContext(natContext);
    }
    ...
    public void onSurfaceChanged(GL10 gl10, int i, int i1) {
        ...
        if (width > 1 && height > 1) {
            ...
            format.setImageType(ImageType.rgba8);
            AvoxWrapper.updateGLESContextTexture(eglContext,textureId[0],format);
        }
    }
    ...
    public void onDrawFrame(GL10 gl10) {
        ...
        copyTexture.renderTex(eglContext);
        // 显示纹理关联的FBO输出
        GLES30.glViewport(0, 0, width, height);
        GLES30.glClear(GLES30.GL_COLOR_BUFFER_BIT);
        GLES30.glClearColor(1.0f, 0.0f, 0.f, 1.f);
        GLES30.glBindFramebuffer(GLES30.GL_READ_FRAMEBUFFER, fboTexture[0]);
