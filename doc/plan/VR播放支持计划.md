...
| # | 任务 | 内容 | 估时 | 状态 |
|---|------|------|------|------|
| 7 | 红蓝 3D | `VrOutMode { mono, anaglyph, sbsPreview }`，anaglyph 在投影后双采样合成（R 取左眼、GB 取右眼），配红青眼镜出立体 | 1.5 天 | ✅ 提前随一期落地（vrProject.comp 分支 + setVrOutMode），待真立体片源验证 |
| 8 | 立体强度 | 视差缩放滑杆（双眼方向偏移量可调），救过强/过弱片源 | 1 天 | 未开始 |
| 9 | 画质补偿 | 投影 remap 后可选挂 FSR/Anime4K（管线现成 pass），补偿上采样发软；按"输出/源像素密度比>1.5×"自动启用或手动开关 | 0.5~1 天 | 未开始（挂载位已预留：vrLayer 在画质层之前） |
...
- **双眼融合提分辨率**：视差偏移是深度相关的（远景亚像素、近景几十像素），利用它须先做逐像素立体配准（= 深度估计，即暂缓项那套机器），理想收益也仅 ~1.2-1.4×；不配准直接融合则近景重影，不可接受。故单眼路线，发软用 FSR/Anime4K 后端补偿（二期 #9）
- **AI 超分画质模式（FSR/Anime4K 之上）**：经典超分对实拍上限低，真提升靠学习型 SR。关键账：SR 只挂投影 remap 之后的**输出端**（视口 ≤4K，成本按输出像素算，1080p 实时可行），不碰 8K 源。主要风险是单帧模型的时域闪烁，须目标 GPU 实测。二期收尾后插 2~3 天 spike（开关 + 2~3 个轻量模型 + avox_onnx，测 fps/闪烁/画质），达标立项"AI 画质模式"（能力分级 + 自动降级 FSR），不达标归档结论
