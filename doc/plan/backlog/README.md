...
**编号与 panvox backlog 对齐,不重排;完成状态以那边打勾为准,此处只标「计划状态」。**
...
M2 媒体源: A-5 (panvox P-4 刮削硬依赖,建议 M1 末期提前启动)
...
- 测试迁移会话(avox 仓):A 类迁移已完成(avox-test 提交 aa5002d),playmatrix 字幕矩阵两边同步中。
  约定:**avox-test 为唯一真源,本仓 tests/ 只读待删**,勿双向演化。
- panvox 会话:评审 ADR-0007 刮削设计(P-4)→ 反推 **A-5 是 avox 侧最不能拖的一项**。
