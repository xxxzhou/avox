...
## 观察项: 包队列统一 200 的网络容忍 (2026-09-24 审查挂账)

- cb85c7b 把点播包队列 1000→200 与直播统一。本地文件无感; 但 HTTP/云盘直链
  等吞吐临界源, demux 领先量从约 33s 压到约 4.3-4.8s(音频包 21-24ms/包钳制),
  网络抖动超过该窗口即 buffering。panvox 已知「直链反复 buffering=吞吐临界」
  场景更易触发。
- 处置: 队列深度只决定断流容忍不加速下载, 不回退; 待弱网/云盘直链实测一次,
  症状复现再议「网络源深度分档」(类似 T2 的本地/网络分档口径)。
