...
**panvox L2 门禁回归(2026-09-17 晚)PASS**:avox-test `switch-size-freeze`
(640x360 ↔ 360x640 换片,probe 帧号递增 + 居中内容区帧差,fz1=2.21 / fz2=39.76,
阈值 1.5)。注意:回归前必须 `panvox/tools/deploy_runtime.ps1` 刷新打包运行时 ——
该轮"红"实为打包目录 avox.dll 落后于本修复,非引擎问题。换片重建空窗 ~2s
(空窗内 GPU 纹理未交付、宿主 CPU 兜底帧泵出错误帧)仍开放,属判据③时序部分。
...
- **实证**: 80s 素材(切换点 40s, 彼时 IO 尚有未读包)上硬解 reset 后正常恢复, 出
  `gen=1→2 640x360→960x540 rebuilt=1` 信号对, 播完 —— DX11VA 重置解码器与新分辨率
