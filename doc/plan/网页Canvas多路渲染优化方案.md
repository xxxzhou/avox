...
> 本文只做方案分析与取舍,不改代码。相关现状见 [播放器Electron](../media/播放器Electron.md)、[OffscreenCanvas自适应分辨率设计](./OffscreenCanvas自适应分辨率设计.md)、[多平台GPU共享](../vulkan/多平台GPU共享.md)。
...
也就是说,**零 CPU 回读的 interop 机制现成**,只是网页用不上。原因见 [播放器Electron.md:94](../media/播放器Electron.md) 的作者实证:
...
- 所以原生 Vulkan/D3D11 纹理无法零拷贝喂给网页 WebGL/WebGPU。这是 Web 平台边界,非本项目问题,也印证了 [播放器Electron.md:94](../media/播放器Electron.md) 作者早年的实证。
...
[播放器Electron.md:137-142](../media/播放器Electron.md) 已实现:
...
| 作者对"DX11 共享句柄+WebGL"的证伪 | `doc/media/播放器Electron.md:94` |
| 原生窗口叠加现成方案 | `doc/media/播放器Electron.md:137-142` |
