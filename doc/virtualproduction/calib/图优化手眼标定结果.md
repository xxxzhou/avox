...
如前面介绍图优化的过程,先需要确定求解的值,在这就是camera2track(也就是原手眼标定求解的值),base2target用于把追踪器坐标系转化到屏幕坐标系下,需要注意的是,这里的track给的位移与现实中的位移没有对应,有个缩放关系,在原来文章[手眼标定算法改进](手眼标定算法改进.md)有讲解过如何改进手眼标定的算法以得到这个缩放值,在这我们也需要求解这个Scale值,这样就有三个值需要求解,然后假定标定过程使用的Track变换相对真实值有细微误差,针对每个Track记录变换也当做一个需要优化的值,这个处理有兴趣的可以详细参考[基于重投影误差最小化的手眼标定](https://ieeexplore.ieee.org/abstract/document/8616862),能正确的处理Track异常记录,经实际测试,对于结果的正确性会有很大提高.
