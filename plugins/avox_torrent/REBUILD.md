...
注意事项:
- /MT 与 avox 全局静态 CRT 对应, 改成 /MD 会 LNK2038 运行时库冲突
- encryption 默认 ON(MSE/PE 协议加密, 可连"强制加密"的 peer)。OpenSSL 取系统安装
  ...
  无 DLL 依赖; 与 avox 主程(httplib/ZLMediaKit 走 MD import lib + 部署 DLL)互不冲突,
...
- ⚠️ avox 当前 Android 加载是"类静态"路径(插件动态扫描在 Android 不可用, 见 plugins/README.md), Android 产物入库供后续静态链入方案使用, 本次插件本身不接 Android
...
| LNK2038 运行时库不匹配 | libtorrent 与 avox 的 /MT|/MD 不一致 |
...
- avox: `cmake/FindLibtorrent.cmake`(查找), `cmake/AVOXOptions.cmake`(find_package 挂接+降级), `plugins/options.cmake`(`AVOX_ENABLE_TORRENT` 开关), `plugins/CMakeLists.txt`(插件清单)
