...
class LamaInpainter : public OnnxModelUser {
 public:
  LamaInpainter();
  ~LamaInpainter();

  // 打开 (经全局缓存取 session; 根据 ModelLevel 自动查找路径)
  bool open(ModelLevel level, bool useGPU = true);
 ...
 private:
  ...
  int inputSize = 512;
  ModelLevel currentLevel = ModelLevel::none;

  // 动态获取的输入名称 (适配不同版本的 LaMa 模型)
